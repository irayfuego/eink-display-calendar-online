...
const uint8_t *getCurrentConditionsBitmap32(const owm_hourly_t &current,
                                             const owm_daily_t   &today)
{
  int id = current.weather.id;
  // OpenWeatherMap indicates sun is up with d otherwise n for night
  ...
  case 212: // Thunderstorm  heavy thunderstorm               11d
  case 221: // Thunderstorm  ragged thunderstorm              11d
    if (!cloudy && day)          {return wi_day_thunderstorm_48x48;}
    if (!cloudy && !day && moon) {return wi_night_alt_thunderstorm_48x48;}
    return wi_thunderstorm_48x48;
  case 230: // Thunderstorm  thunderstorm with light drizzle  11d
  case 231: // Thunderstorm  thunderstorm with drizzle        11d
  case 232: // Thunderstorm  thunderstorm with heavy drizzle  11d
    if (!cloudy && day)          {return wi_day_storm_showers_48x48;}
    if (!cloudy && !day && moon) {return wi_night_alt_storm_showers_48x48;}
    return wi_storm_showers_48x48;
  // Group 3xx: Drizzle
  case 300: // Drizzle       light intensity drizzle          09d
  case 301: // Drizzle       drizzle                          09d
  case 302: // Drizzle       heavy intensity drizzle          09d
  ...
  case 314: // Drizzle       heavy shower rain and drizzle    09d
  case 321: // Drizzle       shower drizzle                   09d
    if (!cloudy && day)          {return wi_day_showers_48x48;}
    if (!cloudy && !day && moon) {return wi_night_alt_showers_48x48;}
    return wi_showers_48x48;
  // Group 5xx: Rain
  case 500: // Rain          light rain                       10d
  case 501: // Rain          moderate rain                    10d
  case 502: // Rain          heavy intensity rain             10d
  case 503: // Rain          very heavy rain                  10d
  case 504: // Rain          extreme rain                     10d
    if (!cloudy && day && windy)          {return wi_day_rain_wind_48x48;}
    if (!cloudy && day)                   {return wi_day_rain_48x48;}
    if (!cloudy && !day && moon && windy) {return wi_night_alt_rain_wind_48x48;}
    if (!cloudy && !day && moon)          {return wi_night_alt_rain_48x48;}
    if (windy)                            {return wi_rain_wind_48x48;}
    return wi_rain_48x48;
  case 511: // Rain          freezing rain                    13d
    if (!cloudy && day)          {return wi_day_rain_mix_48x48;}
    if (!cloudy && !day && moon) {return wi_night_alt_rain_mix_48x48;}
    return wi_rain_mix_48x48;
  case 520: // Rain          light intensity shower rain      09d
  case 521: // Rain          shower rain                      09d
  case 522: // Rain          heavy intensity shower rain      09d
  case 531: // Rain          ragged shower rain               09d
  ...
  case 601: // Snow          Snow                             13d
  case 602: // Snow          Heavy snow                       13d
    if (!cloudy && day && windy)          {return wi_day_snow_wind_48x48;}
    if (!cloudy && day)                   {return wi_day_snow_48x48;}
    if (!cloudy && !day && moon && windy) {return wi_night_alt_snow_wind_48x48;}
    if (!cloudy && !day && moon)          {return wi_night_alt_snow_48x48;}
    if (windy)                            {return wi_snow_wind_48x48;}
    return wi_snow_48x48;
  case 611: // Snow          Sleet                            13d
  case 612: // Snow          Light shower sleet               13d
  case 613: // Snow          Shower sleet                     13d
    if (!cloudy && day)          {return wi_day_sleet_48x48;}
    if (!cloudy && !day && moon) {return wi_night_alt_sleet_48x48;}
    return wi_sleet_48x48;
  case 615: // Snow          Light rain and snow              13d
  case 616: // Snow          Rain and snow                    13d
  case 620: // Snow          Light shower snow                13d
  case 621: // Snow          Shower snow                      13d
  ...
  // Group 7xx: Atmosphere
  case 701: // Mist          mist                             50d
    if (!cloudy && day)          {return wi_day_fog_48x48;}
    if (!cloudy && !day && moon) {return wi_night_fog_48x48;}
    return wi_fog_48x48;
  case 711: // Smoke         Smoke                            50d
    return wi_smoke_48x48;
  case 721: // Haze          Haze                             50d
    if (day && !cloudy) {return wi_day_haze_48x48;}
  ...
  case 771: // Squall        squalls                          50d
    return wi_cloudy_gusts_48x48;
  case 781: // Tornado       tornado                          50d
    return wi_tornado_48x48;
  // Group 800: Clear
  case 800: // Clear         clear sky                        01d 01n
    if (windy)         {return wi_strong_wind_48x48;}
    if (!day && moon)  {return wi_night_clear_48x48;}
    if (!day && !moon) {return wi_stars_48x48;}
    return wi_day_sunny_48x48;
  // Group 80x: Clouds
  case 801: // Clouds        few clouds: 11-25%               02d 02n
    if (windy)         {return wi_strong_wind_48x48;}
    if (!day && moon)  {return wi_night_alt_partly_cloudy_48x48;}
    if (!day && !moon) {return wi_stars_48x48;}
    return wi_day_sunny_overcast_48x48;
  case 802: // Clouds        scattered clouds: 25-50%         03d 03n
  case 803: // Clouds        broken clouds: 51-84%            04d 04n
    if (windy && day)           {return wi_day_cloudy_gusts_48x48;}
    if (windy && !day && moon)  {return wi_night_alt_cloudy_gusts_48x48;}
    if (windy && !day && !moon) {return wi_cloudy_gusts_48x48;}
    if (!day && moon)           {return wi_night_alt_cloudy_48x48;}
    if (!day && !moon)          {return wi_cloud_48x48;}
    return wi_day_cloudy_48x48;
  case 804: // Clouds        overcast clouds: 85-100%         04d 04n
    if (windy) {return wi_cloudy_gusts_48x48;}
    return wi_cloudy_48x48;
  default:
    // maybe this is a new icon in one of the existing groups
    if (id >= 200 && id < 300) {return wi_thunderstorm_48x48;}
    if (id >= 300 && id < 400) {return wi_showers_48x48;}
    if (id >= 500 && id < 600) {return wi_rain_48x48;}
    if (id >= 600 && id < 700) {return wi_snow_48x48;}
    if (id >= 700 && id < 800) {return wi_fog_48x48;}
    if (id >= 800 && id < 900) {return wi_cloudy_48x48;}
    return wi_na_48x48;
  }
} // end getCurrentConditionsBitmap32